...
<header class="page-header">
	...
	<h2><i class="fa fa-money"></i> (Oyun) LiveGames (Tombala) Raporlar</h2>
		...
		<table class="table table-bordered mb-none playgo-table" id="datatable-default">
			...
			<tbody>
				...
				<?php foreach ($raporlar as $rapor) {
					...
					$tur = ( @p('tur') == '1' ) ? 'Ana Bakiye ->' : 'Tombala LiveGames ->';

					$kullanici = kullanici( $rapor['userid'] );

					//
					$game = $db->query("SELECT * FROM livegames_tombala_users WHERE mid = '".$rapor['userid']."'");
					$game = $game->fetch();
