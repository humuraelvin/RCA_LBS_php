...
<div class="navbar-title visible-sm visible-xs">
    <a href="" class="page-title">
        <i class="icon-casino"></i> Canlı Casino <?php  echo $id  ?>
    </a>
</div>


<script type="text/javascript">
 /** Rocks Software's edit codes */

    let table_id = '<?php echo $id ?>';

    function createCanvas(img_path, canvas_id){
        let canvas = document.getElementById(canvas_id);
        let context = canvas.getContext("2d");

        let img = new Image();
        img.src = location.protocol + '//' + location.hostname + img_path;

        img.onload = function () {

            context.shadowColor = 'rgba(20, 189, 140, 0.4)';
            context.shadowBlur = 10;
            context.shadowOffsetX = 0; 
            context.shadowOffsetY = 0; 

            context.drawImage(img, 0, 0, canvas.width - 5, canvas.height - 5);
        };
    }
...
span2.addEventListener('click', () => {
    ...
    function loadGames(sort = 0, loadMoreButton) {
        fetch("https://" + window.location.hostname + "/jsonveri.php?type=live")
            ...
            .then(function (jsonResponse) {
                ...
                dataShown += 24;
                console.log('loadGames res:', gamesData)
    ...
    var gameType = $(element).data("name");
    if (gameType) {
        $.ajax({
            url: 'https://$host/jsonlive.php',
            type: 'GET',
            dataType: 'json',
...
function displayGames(games) {
    ...
    function loadProvider(provider) {
        ...
        let before = document.querySelector('a.active');
        if(before !== null)
            before.classList.remove('active');
        provider.classList.add('active')
        providerGames = gamesData.filter((item) => {
            return item.game_type.toLowerCase() === provider.dataset.name.toLowerCase();
        });
        /* reset the container */
        let container = document.querySelector('#game-container');
        container.innerHTML = '';
...
<section id="main" class="sportsbook_padding newCasino">
    ...
    <div id="large-slider" class="carousel slide mobile-casino" data-ride="carousel"
        ...
        <ol class="carousel-indicators">
            <!--- <?php  var_dump($casino_banners)  ?> --->
            <?php $i = 0; ?>
            <?php foreach($casino_banners as $key => $banner){ ?>
                <li data-target="#large-slider"
                    data-slide-to="<?php echo $i; ?>" class="<?php  if ($i == 0) { echo "active";}  ?>"></li>
                <?php $i++; ?>
            <?php } ?>
        </ol>
        <div class="carousel-inner" role="listbox">
            ...
            ";}  ?>" data-bgcolor="">
            <a href="" target="">
                <img alt="" src="<?php echo $banner->path; ?>" style="border-radius:20px;">
            </a>
                    ...
                    <div class="caption-slide-message">
                        <div class="caption-slide-message-wrp">
                            <span class="message-span"
                                  style="display: none"><?php echo $banner->description; ?></span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="container" style="margin-top: 10px;">
        <div class="categories">
            <div class="no-gutters text-center row" style="display: flex;margin: 0;">
                <div class="col"><a class="" href="#" onclick="location.reload()">
                        <canvas id="baccarat" width="45" height="45"></canvas>
                        <hr>
                        <p class="maxlength">Hepsi</p></a></div>
                        
                <div class="col"><a class="" data-name="roulette" href="#" onclick="loadProvider(this)">
                        <canvas id="roulette" width="45" height="45"></canvas>
                        <hr>
                        <p class="maxlength">Roulette</p></a></div>
                        
                <div class="col"><a class="" data-name="poker"  href="#" onclick="loadProvider(this)">
                        <canvas id="poker" width="45" height="45"></canvas>
                        <hr>
                        <p class="maxlength">Poker</p></a></div>
                        
                <div class="col"><a class="" data-name="blackjack" href="#" onclick="loadProvider(this)">
                        <canvas id="blackjack" width="45" height="45"></canvas>
                        <hr>
                        <p class="maxlength">Blackjack</p></a></div>
                        
                <div class="col"><a class="" data-name="live_other" href="#" onclick="loadProvider(this)">
                        <canvas id="tablegames" width="45" height="45"></canvas>
                        <hr>
                        <p class="maxlength">Dier</p></a></div>
            </div>
        </div>
        <div class="no-gutters categoryBand casino-live-page">
            ...
            <div style="display:flex;align-items: center;align-content: center;flex-wrap: wrap;">
                ...
                <div class="col-8">
                    <div class="input-group filter"><input type="text" class="form-control" placeholder="Arama"
                                                           id="search_games"
                                                           aria-label="Arama" onkeyup="searchGames()">
        ...
        <div class="no-gutters load-more-button">
            ...
            <script type="text/javascript">
                ...
                function searchGames() {
                    ...
                    let games = document.getElementsByClassName('hoverable casino-animation col-12 col-md-4 col-xl-3');
                    ...
                    // Her bir oyun öğesini kontrol et
                    for (let i = 0; i < games.length; i++) {
                        var game_name = games[i].querySelector('.maxlength').textContent.toLowerCase();
                        var gameProvider = games[i].querySelector('img').alt.toLowerCase();

                        // Arama kelimesini oyun adı veya sağlayıcıyla karşılaştır
                        if ((game_name.indexOf(query) > -1 || gameProvider.indexOf(query) > -1)) {
                            found = true;
                            games[i].style.display = 'block'; // Eşleşme bulundu, öğeyi göster
                        } else {
                            games[i].style.display = 'none'; // Eşleşme bulunamadı veya karakter sayısı yetersiz, öğeyi gizle
                        }
                    }

                    if (found === false) {
                        let result = gamesData.filter(item => {
                            return item.game_name.toLowerCase().includes(query) || item.game_section.toLowerCase().includes(query);
                        });
    ...
    <div role="dialog" aria-modal="true" class="fade darkModal modal" tabindex="-1"
		...
		// Oyunu içeren iframe'i bulun
		let iframe = dialog.childNodes[1].childNodes[1].childNodes[1].childNodes[1];

		// Eğer iframe bulunduysa, oyunu durdurun
		if (iframe) {
			iframe.src = ''; // iframe'in src'sini temizleyerek içeriği boşaltabilirsiniz
		}
	});
...
function openGame_org(id){
    table_id = id;
    let game_url = '';
    $.ajax({
        url:"/LiveCasino/impGame/"+table_id,
        success:function(response){
            ...
            if (response.code === "1") {
                ...
                // Eğer tarayıcı penceresi 1200px'den dar ise
                if (window.innerWidth < 1200) {
                    // Yeni bir iframe oluştur ve oyunun URL'sini bu iframe'e ata
                    let iframe = document.createElement("iframe");
                    iframe.src = game_url;
                    // Bu iframe'i belirli bir elemente (örneğin bir div'e) ekleyebilirsiniz
                    // document.getElementById("yourContainerDivId").appendChild(iframe);
                    // ya da direkt olarak yeni sekmede aç
...
if (table_id !== '') {
    openGame(table_id);
            ...
            createCanvas("/images/categories/roulettte.png", "roulette");
            createCanvas("/images/categories/tablegame.png", "tablegames");
            createCanvas("/images/categories/poker.png", "poker");
            createCanvas("/images/categories/baccarat.png", "baccarat");
            createCanvas("/images/categories/blackjack.png", "blackjack");
