                    ...
                    <div class="panel panel-white no-padding-sm">
                        ...
                        <div class="panel-table table-responsive table-mobile">
                            <table class="table table-hover bets-table" id="bet-history-table">
                                ...
                                <tbody>
                                    <?php $i= 1 ?>
                                    <?php foreach($kuponlar as $a => $kupon){ ?>



                                        <tr onclick="couponsdetail('<?php if(isset($kupon["id"])) { echo $kupon["id"];}?>');" class="coupon-filter
                                        <?php if($kupon["durum"] == "1"){ ?> coupon-won
                                        <?php }elseif($kupon["durum"] == 2){  ?> coupon-lost
                                        <?php }elseif($kupon["durum"] == 0){  ?> coupon-pending
                                        <?php }elseif($kupon["durum"] == 3){  ?> coupon-return <?php } ?>">
                                            <td><?php if(isset($kupon["id"])) { echo $kupon["id"];}?></td>
                                            <td><?php echo date("d/m H:i",strtotime($kupon["tarih"])) ?></td>
                                            <td><?php if($kupon["toplam"]>1){ ?> Kombine Kupon<?php }else{?> Tekli Kupon <?php } ?></td>
                                            <td><?php echo nf($kupon["miktar"]) ; ?> TL</td>
                                            <td><?php echo nf($kupon["oran"]) ; ?></td>
                                            <td><?php echo nf($kupon["oran"]*$kupon["miktar"]) ?> TL</td>
                                            <td><b><?php if($kupon["durum"] == "1"){ ?>
                                                        <span style="color:#2bf587;">KAZANDI</span>
                                                        <?php }elseif($kupon["durum"] == 2){  ?>
                                                        <span style="color:#ff9c9c;">KAYBETTİ</span>
                                                        <?php }elseif($kupon["durum"] == 0){  ?>
