<div class="CL-OP RND-OP modal-header">
    ...
    <h4 class="modal-title">#<?php if(isset($kupon["id"])) { echo $kupon["id"];}?> Nolu Kupon Detayı</h4></div>
			...
			<div class="panel-table table-responsive table-mobile">
			    <table class="table table-hover bets-table2" id="bet-history-table">
			        ...
			        <?php foreach($maclar as $m => $mac){ ?>
			            <tr>
				            <td class="bet-history-table-mackodu" data-id="<?php if(isset($mac['mackodu'])) { echo $mac['mackodu'];}?>" style="display: none;"><?php if(isset($mac["mackodu"])) { echo $mac["mackodu"];}?></td>
				            <td class="bet-history-table-date">1</td>
				            <td><?php if(isset($mac["evsahibi"])) { echo $mac["evsahibi"];}?></td>
				            <td><?php if(isset($mac["deplasman"])) { echo $mac["deplasman"];}?></td>
				            <td><?php if(isset($mac["aciklamasi"])) { echo $mac["aciklamasi"];}?></td>
				            <td><?php if(isset($mac["tur"])) { echo $mac["tur"];}?></td>
				            <td><?php echo nf($mac["oran"]) ; ?></td>
