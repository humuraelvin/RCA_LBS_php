<?php if($mac["oynuyormu"]=="1" and $mac["aktifmi"]=="1"){ ?>
...
$url = "https://".$host."/livematchdetail.php?matchid=" . $mac["botid"];

//$url = "https://atabet.bet/livematchdetail.php?matchid=" . $mac["botid"];
//echo $url;
$bukaynak = file_get_contents($url);

$xml = simplexml_load_string($bukaynak);
$kaynak = $xml->MatchDetails->E;
$radarid = (string) $kaynak['BetRadarID'];
$ilkbu = $kaynak->Games->G;
...
$dakika = $mac["dakika"];
		...
		if (!in_array($ilk->attributes()->Name, $filter) ) {


		if(base64_encode($ilk->attributes()->Name) != $turu) {
		...
		echo '<h4>'.$ilk->attributes()->Name.'</h4><i class="pull-right icon-arrow-up"></i></div>';
		echo '<div id="odds-'.$arrowcount.'" class="panel-collapse collapse in">';
			echo '<div class="panel-body">';
				$arrowcount = $arrowcount + '1';
				}

				$turu = base64_encode($ilk->attributes()->Name);
				$a =1;
				$i =0;
				$sayi = count($ilk);
				...
				if ($sayi == 3) {
					...
					foreach($ilk->R as $val){
						...
						if ($val->attributes()->GameIsVisible == 0) {
							$click = '';
						...
						$click = 'onclick="addsliplive(\''.sifrele($mac["id"].'___'.$ilk->attributes()->Name.'___'.$val->attributes()->Name.'___'.$oran).'\');"';
					...
					echo '<div class="hvr-shutter-out-vertical col-sm-4" '.$click.' ><span>'.$oran.'</span>'.$val->attributes()->Name.'</div>';
				}
				echo '</div>';
			...
			echo '<div class="row" data-something="3" >';
				$oz = 0;
				foreach($ilk->R as $val){
//    echo "Oran Adı: " . $val->attributes()->Name . " - Oran Değeri: " . $val->attributes()->O0 . "<br>";
//echo "Oran Durum : " . $val->attributes()->GameIsVisible;
				if ($val->attributes()->GameIsVisible == 0) {
				$click = '';
				$oran = '<i  class="fa fa-lock"></i>';
				}
				...
				echo '<div class="hvr-shutter-out-vertical col-sm-6" '.$click.' ><span>'.$oran.'</span>'.$val->attributes()->Name.'</div>';
			$oz = $oz + 1;
			if ($oz == 2)
			{
			echo '</div>';
