                    ...
                    <div class="panel panel-white no-padding-sm">
                        ...
                        <div class="panel-table table-responsive table-mobile">
                            <table class="table table-hover bets-table" id="bet-history-table">
                                ...
                                <tbody>
                                    {php}$i = 1;{/php}
                                    {foreach from="$detaylar" item="detay" key="a"}
                                        <tr>
                                            <td>Bahis</td>
                                            <td>{nf($detay["oncekibakiye"])} TL</td>
                                            <td>{nf($detay["tutar"])} TL</td>
                                            <td>{nf($detay["sonrakibakiye"])} TL</td>
                                            <td>{php} echo str_replace("( İşlemi Yapan : admin)","",$detay["islemad"]) {/php}</td>
                                            <td>{php}echo date("H:i d/m/y",strtotime($detay["tarih"])){/php}</td>
                                        </tr>
                                        {php} $i++;{/php}
                                    {/foreach}
                                </tbody>
