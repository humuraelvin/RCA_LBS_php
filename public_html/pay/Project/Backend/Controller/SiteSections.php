<?php

  Namespace Project\Controller;

  Use Helper\Database\DBGetCount as DBGetCount;
  Use Library\Database as DB;
  Use Library\Paginator as Paginator;
  Use Project\Model\SiteSections as Model;

  Class SiteSections extends Model {

    /**
    * CRUD
    */

    Public Static Function create()
    {
      if(isset($_POST['add']) and PSLUG2=='add')
      {
        if(!DBGetCount::create(self::slug()))
        {
          $insert = DB::insert(self::$tableName, self::postData());
          if($insert)
          {
            $insertWebsite = DB::insert("sta_sections_website", self::postData("website"));
            $insertMeta    = DB::insert("sta_sections_website_meta", self::postData("meta"));
            alert("success", "Başarılı!", "İşleminiz başarıyla tamamlandı.");
            go(PATH_DASHBOARD."/siteSections",1);
          }
          else
          {
            alert("error", "Hata!", "İşlem gerçekleştirilirken teknik bir sorun oluştu.");
          }
        }
        else
        {
          alert("error", "Hata!", "Daha önce benzer bir içerik eklendi. Başlığı kontrol edin.");
        }
      }
    }

    Public Static Function update($identity)
    {
      if(isset($_POST['edit']) and PSLUG2=='edit')
      {
        $update = DB::update(self::$tableName, self::postData(), $identity."='".g("id")."'");
        if($update)
        {
          $updateWebsite = DB::update("sta_sections_website", self::postData("website"), "slug='".g("id")."'");
          $updateMeta    = DB::update("sta_sections_website_meta", self::postData("meta"), "slug='".g("id")."'");
          alert("success", "Başarılı!", "İşleminiz başarıyla tamamlandı.");
          go(PATH_DASHBOARD."/siteSections/edit?id=".self::slug().($_GET['lang'] ? "&lang=".$_GET['lang'] : NULL),0.8);
        }
        else
        {
          alert("error", "Hata!", "İşlem gerçekleştirilirken teknik bir sorun oluştu.");
        }
      }
    }

    Public Static Function delete($identity)
    {
      if(PSLUG2=='delete')
      {
        $delete = DB::delete(self::$tableName, $identity."=?", array(g("id")));
        if($delete)
        {
          $deleteWebsiteSection = DB::delete("sta_sections_website", "slug=?", array(g("id")));
          alert("success", "Başarılı!", "İşleminiz başarıyla tamamlandı.");
          go(PATH_DASHBOARD."/siteSections",1);
        }
        else
        {
          alert("error", "Hata!", "İşlem gerçekleştirilirken teknik bir sorun oluştu.");
        }
      }
    }

    Public Static Function deleteSelected($identity)
    {
      if(count($_POST['item']))
      {
        for ($i=0; $i < count($_POST['item']) ; $i++) {
          $delete = DB::delete(self::$tableName, $identity."=?", array($_POST['item'][$i]));
          $deleteWebsiteSection = DB::delete("sta_sections_website", "slug=?", array($_POST['item'][$i]));
        }
        if($i)
        {
          alert("success", "Başarılı!", "İşleminiz başarıyla tamamlandı.");
        }
      }
    }

    /**
    * METHODS
    */

    Public Static Function single()
    {
      $query = DB::single("SELECT * FROM sta_sections_dashboard WHERE slug=?", array(PSLUG1));
      return $query;
    }

    Public Static Function meta()
    {
      $query = DB::single("SELECT * FROM sta_sections_website_meta WHERE slug=?", array(g("id")));
      return $query;
    }

    Public Static Function edit()
    {
      $query = DB::single("SELECT * FROM ".self::$tableName." WHERE slug=?", array(g("id")));
      return $query;
    }


    /**
    * PAGENATION
    */

    Public Static Function total()
    {
      $query = DB::rowCount("SELECT id FROM ".self::$tableName." ".self::conditions(), self::paramaters());
      return $query;
    }

    Public Static Function fetch()
    {
      $query = DB::fetch("SELECT ".self::$tableSelect." FROM ".self::$tableName." ".self::conditions()." ".self::orderby(), self::paramaters());
      return $query;
    }

  }
