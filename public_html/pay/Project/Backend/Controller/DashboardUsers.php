<?php
  ...
  Use Project\Model\DashboardUsers as Model;

  Class DashboardUsers extends Model {
    ...
    */
    ...
    Public Static Function create()
    {
      if(isset($_POST['add']) and PSLUG2=='add')
      {
        ...
        if($insert)
        {
          ...
          go(PATH_DASHBOARD."/dashboardUsers",1);
        }
        else
        {
    ...
    Public Static Function update($identity)
    {
      if(isset($_POST['edit']) and PSLUG2=='edit')
      {
        ...
        {
          alert("success", "Başarılı!", "İşleminiz başarıyla tamamlandı.");
          go(PATH_DASHBOARD."/dashboardUsers/edit?id=".g("id"),1);
        }
        else
        {
          alert("error", "Hata!", "İşlem gerçekleştirilirken teknik bir sorun oluştu.");
    ...
    */
    ...
    {
      $query = DB::single("SELECT name FROM sta_administrator_permissions WHERE id=?", array($id));
      return $query->name;
    }
