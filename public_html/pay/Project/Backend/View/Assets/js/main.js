...
(function($) {
   ...
   * ------------------------------------------------------------------------- */
   ...
   var photoID = this.id;
   $.ajax({
     url: '../../Module/DashboardAjax.php?section=photo-delete',
     type: 'POST',
     data: { photoID:photoID },
     success: function(response){
      $(el).closest('#photo-delete-area').fadeOut(400, function(){
        $(this).remove();
      });
     }
   });
  ...
  $('.photo-ord-update').change(function(){
    var photoID  = this.id;
    var order    = this.value;
    $.ajax({
      url: '../..//Module/DashboardAjax.php?section=photo-update',
      type: 'POST',
      data: { photoID:photoID, order:order},
      success: function(response){
      //$('#photo-update-return').html(response);
      }
    });
  });
   ...
   * SLIDER UPDATE
   * ------------------------------------------------------------------------- */
   $('.slider-delete-link').click(function(){
   var el = this;
   var sliderID = this.id;
   $.ajax({
     url: '../..//Module/DashboardAjax.php?section=slider-delete',
     type: 'POST',
     data: { sliderID:sliderID },
     success: function(response){
       $(el).closest('#slider-delete-area').fadeOut(400, function(){
         $(this).remove();
       });
     }
    });
   ...
   $('.slider-ord-update').change(function(){
    var sliderID  = this.id;
    var order    = this.value;
    $.ajax({
      url: '../..//Module/DashboardAjax.php?section=slider-update',
      type: 'POST',
      data: { sliderID:sliderID, order:order},
      success: function(response){
      //$('#photo-update-return').html(response);
      }
    });
   ...
   $('.slider-content-update').change(function(){
    var sliderID  = this.id;
    var content    = this.value;
    $.ajax({
      url: '../..//Module/DashboardAjax.php?section=slider-content-update',
      type: 'POST',
      data: { sliderID:sliderID, content:content},
      success: function(response){
      //$('#photo-update-return').html(response);
      }
    });
   ...
   * "TERM" UPDATE
   * ------------------------------------------------------------------------- */
   $('.term-section-update').change(function(){
    var id      = this.id;
    var content = this.value;
    $.ajax({
      url: '../..//Module/DashboardAjax.php?section=term-section-update',
      type: 'POST',
      data: { termID:id, termSection:content},
      success: function(response){
        //..
      }
    });
   ...
   $('.term-define-update').change(function(){
     var id      = this.id;
     var content = this.value;
     $.ajax({
       url: '../..//Module/DashboardAjax.php?section=term-define-update',
       type: 'POST',
       data: { termID:id, termDefine:content},
       success: function(response){}
     });
   });

   $('.term-tr-update').change(function(){
     var id      = this.id;
     var content = this.value;
     $.ajax({
       url: '../..//Module/DashboardAjax.php?section=term-tr-update',
       type: 'POST',
       data: { termID:id, termContent:content},
       success: function(response){}
     });
   });

   $('.term-en-update').change(function(){
     var id      = this.id;
     var content = this.value;
     $.ajax({
       url: '../..//Module/DashboardAjax.php?section=term-en-update',
       type: 'POST',
       data: { termID:id, termContent:content},
       success: function(response){}
     });
  ...
  $(function() {
    ...
    if ($recordsListView.length) {
      $recordsListView.DataTable({
        ...
        dom: '<"topbar"<"toolbar"><"right"li>><"table-responsive table-hover"t>p',
        order: [],
        columnDefs: [{
     ...
     * RECORDS LIST FULL
     * ------------------------------------------------------------------------- */
    var $recordsListFull = $('.records--list--full'),
      $recordsListFullView = $('#recordsListFullView');

    if ($recordsListFullView.length) {
      $recordsListFullView.DataTable({
        responsive: true,
        pageLength: 100,
        language: {
          "lengthMenu": "View _MENU_ records"
      ...
      $recordsListFull.find('.toolbar').text($recordsListFull.data('title'));
    }
