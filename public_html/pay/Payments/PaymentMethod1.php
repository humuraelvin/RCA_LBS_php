<?php
  ...
  {
    go(PATH.'/Payments/PaymentMethod1.php?id='.$_POST['user'].'&price='.$_POST['amount']);
  }

  $method = Library\Database::single("SELECT * FROM sta_system_banks WHERE bankID=?", array(1));

  if(isset($_POST["send"]))
  {
    if(Library\Database::rowCount("SELECT * FROM sta_data_user WHERE username=?", array(post("username"))))
    {
      $insert     = Library\Database::insert("sta_data_user_payments", ['user_id' => Helper\Database\DBGetID::userUsername(post("username"), 'user_id'), 'payment_method' => 1, 'payment_amount' => $_POST['amount'], 'payment_status' => 0]);
      if($insert)
      {
        $postResponse = "
          <script>
    ...
    else
    {
      ...
      $insert     = Library\Database::insert("sta_data_user_payments", ['user_id' => $userID, 'payment_method' => 1, 'payment_amount' => $_POST['amount'], 'payment_status' => 0]);
      if($insert)
      {
        $postResponse = "
          <script>
      ...
      <div id="app" class="case">
        <div class="card payment-card">
          <div class="card-header font-weight-bolder">
            <a href="javascript:;" onclick="Route.push('main')" class="font-weight-bold text-dark pr-2">
              <i class="ion-chevron-left"></i>
            </a> Papara <span class="font-weight-normal">ile yatırım yap</span>
          </div>
          <div class="card-body">

            <div class="alert alert-warning" role="alert"> Anında Papara ile kolayca ve hızlıca para yatırabilir ve çekebilirsiniz. </div>
            <hr>
