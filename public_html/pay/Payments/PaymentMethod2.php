<?php
  ...
  {
    go(PATH.'/Payments/PaymentMethod2.php?id='.$_POST['user'].'&price='.$_POST['amount']);
  }
  ...
  if(isset($_POST["send"]))
  {
    ...
    {
      $insert     = Library\Database::insert("sta_data_user_payments", ['user_id' => Helper\Database\DBGetID::userUsername(post("username"), 'user_id'), 'payment_method' => 2, 'payment_amount' => $_POST['amount'], 'payment_status' => 0]);
      if($insert)
      {
        $postResponse = "
          <script>
    ...
    else
    {
      ...
      $insert     = Library\Database::insert("sta_data_user_payments", ['user_id' => $userID, 'payment_method' => 2, 'payment_amount' => $_POST['amount'], 'payment_status' => 0]);
      if($insert)
      {
        $postResponse = "
          <script>
      ...
      <div id="app" class="case">
        <div class="card payment-card">
          <div class="card-header font-weight-bolder">
            ...
            </a> Payfix <span class="font-weight-normal">ile yatırım yap</span>
          </div>
          <div class="card-body">

            <div class="alert alert-warning" role="alert"> Anında payfix ile kolayca ve hızlıca para yatırabilir ve çekebilirsiniz. </div>
            <hr>
